...
Version 1.28 (2008-05-14)
- added a check digit to Postnet barcode
...
var
  ...
  s : string;
begin
  s := CheckSumPostnet(FText);    // calculate check digit

  result := '51';   // Start bar

  for i:=1 to Length(s) do
  begin
    result := result + tabelle_PostNet[s[i]];
  end;
  result := result + '5';  // Stop bar
end;
